...
class PyMongo():
    ...
    def consulta_general_productos(self, collection, filtro_registros = None, atributos = None):
        response = {"status": False, "resultado": []}
        lista_pipelines = []
        ...
        self.MONGO_RESPUESTA = self.MONGO_CLIENT[self.MONGO_DATABASE][collection].aggregate(lista_pipelines)
        if self.MONGO_RESPUESTA:
            response["status"] = True
            for reg in self.MONGO_RESPUESTA:
                response["resultado"].append(reg)
    ...
    def obtener_promedio_estudiantes(self, collection = 'kardex'):
        response = {"status": False, "resultado": []}
        self.MONGO_RESPUESTA = self.MONGO_CLIENT[self.MONGO_DATABASE][collection].aggregate(
            [
                {
        "$group":
            {
