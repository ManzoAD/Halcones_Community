...
function enviarDatos() {
    const url = 'generarPedido.php';
    const data = new FormData();

    /* data.append('Nombre', 'Leonardo Martínez González');*/
    data.append('idCliente', 1000);
    data.append('metodoDePago', 5000);
    data.append('idDomicilio', 10);
    data.append('jsonCarrito', JSON.stringify(productosCarrito));
    ...
    fetch(url, {
        ...
        .then(respuesta => {
            if (respuesta.ok) {
                ...
                throw "ERROR: en la llamada al archivo generarPedido.php";
        })
        .then(function(texto) {
            //console.log(texto);
